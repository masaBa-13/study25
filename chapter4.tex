...
図\ref{fig:overlap}を見ると、促進区域内側の南側には利用頻度が高いグリッドが複数存在しているが、緑色の点で示した投縄開始地点は促進区域から少なくとも3km離れており、直接的な重複は確認されなかった。
...
\begin{figure}[H]
  \centering
   \includegraphics[width=12cm]{images/fig4_12_effort_vs_catch_scatter}
...
\subsection{燃油効率の推移}
投入された燃油に対する生産性を示す燃油効率（$E_o$）の算出結果を表\ref{tab:fuel_efficiency}に示す。

\begin{table}[htbp]
  \centering
  \caption{漁期ごとの燃油効率（漁獲重量 / 給油量）}
  \label{tab:fuel_efficiency}
  \begin{tabular}{lccc}
    \hline
    漁期 & 総漁獲量 (kg) & 総給油量 (L) & 燃油効率 ($E_o$: kg/L) \\
    \hline
    2023年 & 46,814 & 72,436 & 0.646 \\
    2024年 & 55,762 & 65,453 & 0.852 \\
    2025年 & 59,104 & 72,174 & 0.819 \\
    \hline
  \end{tabular}
\end{table}

表\ref{tab:fuel_efficiency}より、燃油効率は2023年の0.646 kg/Lから2024年には0.852 kg/Lへと上昇したものの、2025年は0.819 kg/Lとなり、前年（2024年）と比較して数値の低下が確認された。
各要素の変化を見ると、2025年の総漁獲量は2024年比で約6.0\%（+3,342 kg）増加した。一方で、同期間における総給油量は約10.3\%（+6,721 L）増加した。
結果として、2025年は漁獲量の増加率よりも給油量の増加率が上回ったため、単位燃油あたりの燃油効率は低下する結果となった。
ただし、燃油給油量の算出には漁協の管理している給油記録を用いており、29隻中の27隻分のデータが含まれているものの、全隻分のデータが揃っていないため、これらの数値は参考値として解釈する必要がある。

\subsection{月別燃油効率の変動特性分析}
年間を通じての効率の変動を把握するため、月別の燃油効率の推移を比較した（図\ref{fig:monthly_fuel_efficiency}）。

\begin{figure}[H]
  \centering
  \includegraphics[width=14cm]{images/fig_monthly_fuel_efficiency_catch}
  \caption{月別燃油効率の推移（2023年〜2025年）}
  \label{fig:monthly_fuel_efficiency}
\end{figure}

図\ref{fig:monthly_fuel_efficiency}を参照すると、各年とも7月が効率のピークとなる点は共通している。2023年は漁獲量、燃油効率の高い時期が7月〜9月まで続き、その後急激に悪化している。2024年は8月まで効率が維持された後、9月以降急激に低下する傾向が見られた。対して2025年は、7月をピークに8月に効率が大きく低下するものの、10月〜11月にかけて比較的高い水準へ好転していることが特徴的である。
...
\begin{figure}[H]
  \centering
   \includegraphics[width=14cm]{images/fig_monthly_catch_cpue}
  \caption{月別CPUEの推移（2024年〜2025年）}
  \label{fig:monthly_cpue}
\end{figure}

図\ref{fig:monthly_cpue}を参照すると、2024年は7月がCPUEのピークとなり、10月まで減少した後、11月にかけて若干の回復が見られた。2025年は8月が漁獲量が少ないものの、昨年の3倍以上のCPUEであり、10月〜11月のCPUEも2024年と比較して高い水準で推移していることが確認された。